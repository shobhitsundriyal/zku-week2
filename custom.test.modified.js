...
describe('Custom Tests', function () {
  ...
  it('[assignment] ii. deposit 0.1 ETH in L1 -> withdraw 0.08 ETH in L2 -> assert balances', async () => {
    ...
    const omniBridgeBalance = await token.balanceOf(omniBridge.address)//0.08
    ...
    expect(tornadoPoolBalance).to.be.equal(aliceChangeUtxo.amount)//0.02
  })
  ...
  it('[assignment] iii. see assignment doc for details', async () => {
    ...
    // Bob withdraws all part of his funds from the shielded pool
    ...
    // Alice withdraws all part of his funds from the shielded pool
    ...
    expect(bobBalance).to.be.equal(bobWithdrawAmount)//0.06
    expect(bobBalance).to.be.equal(utils.parseEther('0.06'))

    const omniBridgeBalance = await token.balanceOf(omniBridge.address)
    expect(omniBridgeBalance).to.be.equal(aliceWithdrawAmount)//0.07
